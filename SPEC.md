...
| Recipes disagree about the same dish        | "How long should the bolognese simmer?" with two bolognese variants in the corpus giving different times | `200`, `answer` reports both values attributed to their recipes, `sources` has ≥ 2 entries, `conflicts` has ≥ 1 entry describing the disagreement. |
...
**Target for 1,000 questions:** TBD. To be filled in from the mean
`cost_usd` of the baseline eval run multiplied by 1,000.
